...
public class UnidadesUsuariosEventHandler {
	...
	public void handleUnidadesBeforeCreate(UnidadesUsuarios uu) {
		...
		try {
			...
			throw new RuntimeException(
					"Fallo Antes de Crear UnidadesUsuarios Validando Autorización: " + e.getMessage());
		}
	...
	public void handleUnidadesAfterCreate(UnidadesUsuarios uu) {
		...
		try {
			...
			throw new RuntimeException(
					"Fallo Despues de Crear UnidadesUsuarios Validando Autorización: " + e.getMessage());
		}

		Usuarios usu = usuRepo.findById(uu.getUsuario().getId()).get();
		Unidades uni = uniRepo.findById(uu.getUnidad().getId()).get();

		ActualizarTraza(val, uu.getId(), 4, 1,
				"Fue Asignado el Usuario " + usu.getTip() + " a la Unidad " + uni.getDenominacion(),
	...
	public void handleUnidadesBeforeDelete(UnidadesUsuarios uu) {
		...
		try {
			...
			throw new RuntimeException(
					"Fallo Antes de Eliminar UnidadesUsuarios Validando Autorización: " + e.getMessage());
		}
	}
	...
	public void handleUnidadesAfterDelete(UnidadesUsuarios uu) {
		...
		try {
			...
			throw new RuntimeException(
					"Fallo Despues de Eliminar UnidadesUsuarios Validando Autorización: " + e.getMessage());
		}

		try {
			ActualizarTraza(val, uu.getId(), 4, 2,
					"Fue Desasignado el Usuario " + uu.getUsuario().getTip() + " de la Unidad "
							+ uu.getUnidad().getDenominacion(),
		...
		try {
			List<Permisos> permisos = perRepo.findByUsuarios(uu.getUsuario());
		...
		try {
			...
			log.error("Fallo al Eliminar la Unidad en el Usuario");
			log.error(e.getMessage());
